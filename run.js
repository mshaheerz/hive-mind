...
function classifyRisk(action = "") {
  ...
  if (
    /deploy|publish|release|migration|drop|truncate|delete from|secret write|secretsmanager put|keyvault set|destroy|rm -rf|git reset --hard/.test(
      text,
    )
  ) {
    return "high";
...
function hydrateWorkspaceFromOutputs(projectName) {
  ...
  if (written.length) {
    ...
    ensureWorkspaceScaffold(
      projectName,
      status.preferredStack || status.stack || status.template || "",
    );
...
class AutonomousRunner {
  ...
  _syncProviderState() {
    ...
    if (previous && previous !== current) {
      ...
      log(
        "system",
        `Provider changed (${previous} -> ${current}). Resetting agent schedules for immediate cycle activity.`,
      );
    }
  ...
  async start() {
    ...
    this.intervalId = setInterval(async () => {
      if (this.cycleInProgress) {
        log(
          "system",
          "Previous cycle still running; skipping this interval tick.",
        );
        return;
      }
      await this.cycle();
  ...
  _reconcileOverdue(overdue) {
    ...
    for (const task of overdue) {
      if (this._isStaleDeadline(task)) {
        ...
        log(
          "system",
          `Cleared stale deadline: ${task.projectName}::${task.stage} (already reached).`,
        );
        continue;
      }
  ...
  async checkDeadlines() {
    ...
    for (const task of overdue) {
      const hoursLate = ((Date.now() - new Date(task.dueAt)) / 3600000).toFixed(
        1,
      );
      log(
        "apex",
        `  • "${task.projectName}" → ${task.stage} (${task.agent.toUpperCase()} is ${hoursLate}h late)`,
      );
  ...
  _agentOpsSummary(agent) {
    ...
    const minsSince = last
      ? Math.floor((Date.now() - new Date(last).getTime()) / 60000)
      : null;
  ...
  _projectOpsSummary(limit = 2) {
    const active = getActiveProjectsWithStatus().sort(
    ...
    if (deferredCount > 0)
      lines.push(`Deferred projects: ${deferredCount} (capacity gate).`);

    for (const { name, status } of prioritized) {
      const stage = String(status.stage || "unknown");
      const owner =
        STAGE_RESPONSIBLE_AGENT[stage] || status.stageOwner || "none";
      const blocked = status.blockedReason?.message
        ? `blocked=${status.blockedReason.message}`
        : "blocked=none";
      lines.push(
        `${name}: stage=${stage} owner=${String(owner).toUpperCase()} ${blocked}`,
      );
    }
    return lines;
  }
  ...
  _overdueOpsSummary() {
    ...
    for (const task of overdue.slice(0, 4)) {
      ...
      lines.push(
        `${task.projectName}:${task.stage} owner=${task.agent.toUpperCase()} late=${hoursLate}h`,
      );
    }
    return lines;
  }
  ...
  async processCeoBridge() {
    ...
    for (const msg of pending) {
      ...
      const asksOps =
        /progress|priority|priorit|which project|most priority|blocked|overdue|deadline|stuck|current stage|what is going on|what's going on|what is the progress|taken/i.test(
          lower,
        );
      ...
      if (strictOrder && wakeTargets.length) {
        ...
        this.state.save();
        responseParts.push(
          `Strict order accepted. Forced wake: ${wakeTargets.map((a) => a.toUpperCase()).join(", ")}.`,
        );
      }
      ...
      if (asksWhy) {
        ...
        responseParts.push(
          ...targets.map((agent) => this._agentOpsSummary(agent)),
        );
      }
...
${text}`,
          ...
          responseParts.push(
            String(convo || "").trim() ||
              "Acknowledged. I am monitoring execution and will enforce discipline.",
          );
        } catch (err) {
          responseParts.push(
            "Acknowledged. I am monitoring execution and will enforce discipline.",
  ...
  async cycle() {
    ...
    const dueAgents = Object.keys(dueSnapshot).filter(
      (name) => dueSnapshot[name],
    );
    if (dueAgents.length) {
      log(
        "system",
        `Due agents this cycle: ${dueAgents.map((a) => a.toUpperCase()).join(", ")}`,
      );
    } else {
    ...
    try {
      ...
      for (const name of getProjects()) {
        ...
        if (removedArtifacts > 0) {
          log(
            "system",
            `Cleaned ${removedArtifacts} malformed workspace artifact file(s) for "${name}".`,
          );
        }
        if (!hasRealProjectFiles(name)) {
          ...
          if (count)
            log(
              "system",
              `Backfilled ${count} workspace file(s) for "${name}" from implementation output.`,
            );
        }
        ...
        if (status.stage === "complete" && !hasRealProjectFiles(name)) {
          setProjectStatus(name, {
            ...
            completionRevokedReason:
              "No real source files materialized in workspace",
          });
          log(
            "system",
            `Reopened "${name}": completion revoked because no real source files exist.`,
          );
        }
      }
      ...
      if (capacityFull) {
        log(
          "system",
          `Capacity mode: ${activeProjects.length} active project(s). Holding new intake until fewer than ${MAX_ACTIVE_PROJECTS}.`,
        );
      }
  ...
  async novaGeneratesIdeas() {
    ...
    const existingIdeas =
      this.duplicates.index.ideas.map((i) => i.title).join(", ") || "none yet";
...
Do NOT propose anything similar to these. Think of genuinely new, useful developer tools or automation projects.`;
    ...
    for (const proposal of proposals) {
      ...
      // Duplicate check
      const dupCheck = this.duplicates.isDuplicate(
        proposal.title,
      ...
      if (dupCheck.isDuplicate) {
        log(
          "nova",
          `⚠ Skipping "${proposal.title}" — too similar to "${dupCheck.similarTo.title}" (${(dupCheck.similarity * 100).toFixed(0)}% match)`,
        );
      ...
      queue.push({
        ...
        acceptanceSignals: Array.isArray(proposal.acceptanceSignals)
          ? proposal.acceptanceSignals.slice(0, 8)
          : [],
    ...
    if (newCount === 0) {
      ...
      log(
        "nova",
        `✓ Submitted ${newCount} new proposal(s) for SCOUT review. Going to rest.`,
      );
    }
    return true;
  }
  ...
  async scoutValidatesProposals() {
    ...
    const pending = queue.filter(
      (p) => p.status === "pending_scout" || p.status === "needs_revision",
    );
    ...
    for (const proposal of pending) {
      ...
      try {
        ...
        this.discussion.post(
          threadId,
          "nova",
          `I propose: ${proposal.description}. Reasoning: ${proposal.reasoning}`,
        );
        this.discussion.post(
          threadId,
          "scout",
        ...
        log(
          "scout",
          `✓ Research complete for "${proposal.title}". Forwarding to APEX.`,
        );
      } catch (err) {
        log(
          "scout",
          `✗ Failed to research "${proposal.title}": ${err.message}`,
        );
      }
    }
  ...
  async apexReviewsAndDecides(opts = {}) {
    ...
    if (blockApprovals) {
      log(
        "apex",
        `Capacity gate active (${activeCount}/${MAX_ACTIVE_PROJECTS} active). Holding ${pending.length} pending APEX decision(s).`,
      );
      return false;
    }
    ...
    for (const proposal of pending) {
      try {
        ...
        const thread = this.discussion.getThread(
          proposal.threadId || proposal.id,
        );
        ...
        if (decision.decision === "APPROVED") {
          ...
          this.discussion.closeThread(
            proposal.id,
            `REJECTED: ${decision.reasoning}`,
          );
          log(
            "apex",
            `❌ REJECTED: "${proposal.title}" — ${decision.reasoning}`,
          );
        } else {
          ...
          this.discussion.post(
            proposal.id,
            "apex",
            `Revision needed: ${decision.feedback}`,
          );
  ...
  async apexChecksProjectHealth() {
    ...
    for (const task of overdue) {
      ...
      log(
        "apex",
        `  → Reassigning ${task.stage} to ${task.agent.toUpperCase()} (deadline reset)`,
      );
    }
  }
...
`;
    ...
    ensureWorkspaceScaffold(
      slug,
      proposal.preferredStack || proposal.stack || proposal.template || "",
    );
  ...
  async advanceProjects(dueSnapshot = {}) {
    ...
    if (deferred.length) {
      log(
        "system",
        `Project priority active: processing oldest ${MAX_ACTIVE_PROJECTS}, deferring ${deferred.length} newer project(s).`,
      );
    }
      ...
      while (hops < MAX_STAGE_HOPS_PER_CYCLE) {
        ...
        if (!stageInfo || !stageInfo.agentKey) {
          if (currentStatus.stage === "launch" && !hasRealProjectFiles(name)) {
            log(
              "system",
              `Completion blocked for "${name}": no real source files in projects/${name}/workspace`,
            );
            break;
          }
        ...
        if (!dueSnapshot[stageInfo.agentKey]) {
          ...
          if (forceNow) {
            log(
              "apex",
              `Strict order: waking ${stageInfo.agentKey.toUpperCase()} now for "${name}" (${currentStatus.stage}); skipped wait ${minsLeft}m.`,
            );
    ...
    if (!progressed && blocked.length) {
      const lines = blocked
        .map(
          (b) =>
            `${b.project}:${b.stage} waiting for ${b.agent.toUpperCase()} (${b.minsLeft}m)`,
        )
        .join(" | ");
  ...
  async runProjectStage(projectName, stage, stageInfo, status) {
    ...
    try {
      ...
      const decisionApproved =
        APPROVAL_MODE !== "risk_based" || riskClass !== "high";
      if (!decisionApproved) {
        appendRunEvidence(
          projectName,
          run.runId,
        ...
        setProjectStatus(projectName, {
          blockedReason: {
            ...
            message: `High risk action blocked for ${stage}. Await APEX decision.`,
          },
      ...
      switch (stage) {
        ...
        case "architecture":
          ...
          const remediationItems = [
            ...(Array.isArray(status.lensActionItems)
              ? status.lensActionItems
              : []),
            ...(Array.isArray(status.pulseActionItems)
              ? status.pulseActionItems
              : []),
          ];
          const mandatoryFixMap =
            Array.isArray(status.lensActionItems) &&
                ...
                blockedReason:
                  check.failed > 0
                    ? {
                        ...
                        message:
                          "Workspace checks failed. FORGE must fix syntax/runtime issues.",
                      }
                    : null,
                workspaceCheck: {
        ...
        case "implementation":
          ...
          if (
            ...
            log(
              "lens",
              `⚠ Code issues found in "${projectName}". Sending back to FORGE.`,
            );
            ...
            const hasRepeatedCritical = lensActionItems.some(
              (item) =>
                item.severity === "critical" &&
                Number(repeats[item.id] || 0) >= MAX_LENS_REJECTS_BEFORE_BYPASS,
            );
            const shouldBypass =
              nextRejectCount >= MAX_LENS_REJECTS_BEFORE_BYPASS ||
              hasRepeatedCritical;
            ...
            if (shouldBypass) {
              ...
              log(
                "apex",
                `🚨 Escalation: "${projectName}" rejected by LENS ${nextRejectCount} times. Auto-bypassing to PULSE review.`,
              );
              this.deadlines.complete(projectName, stageInfo.next);
              finalizeRunArtifact(projectName, run.runId, {
                outcome: "approved",
                rationale:
                  "Escalation policy bypassed repeated rejection loop to progress pipeline.",
                risk: riskClass,
                fromAgent: "lens",
                toAgent: "pulse",
                project: projectName,
            ...
            finalizeRunArtifact(projectName, run.runId, {
              outcome: "rejected",
              rationale:
                "LENS found critical issues and sent implementation back to FORGE.",
              risk: riskClass,
              fromAgent: "lens",
              toAgent: "forge",
              project: projectName,
              stage,
              summary: lensSummary,
              requiredActions: lensActionItems.map(
                (item) => `${item.id}: ${item.requirement}`,
              ),
              modelInfo: {
                provider:
                  this.state.state.llmProvider || process.env.LLM_PROVIDER,
        ...
        case "review":
          ...
          writeOutput(projectName, "tests.md", output);
          {
            ...
            const generatedTestFiles = materializeForgeFiles(
              projectName,
              output,
              {
                pathAllowList: [
            ...
            if (!testRun.passed) {
              ...
              log(
                "pulse",
                `⚠ Tests/checks failed for "${projectName}". Sending back to FORGE.`,
              );
              ...
              finalizeRunArtifact(projectName, run.runId, {
                outcome: "rejected",
                rationale:
                  "PULSE tests failed; implementation sent back for remediation.",
                risk: riskClass,
                fromAgent: "pulse",
                toAgent: "forge",
                project: projectName,
            ...
            log(
              "pulse",
              `✓ PULSE accepted "${projectName}" (${testRun.summary}).`,
            );
          }
          break;
      ...
      const updates = {
        ...
        stageOwner: STAGE_RESPONSIBLE_AGENT[stageInfo.next] || null,
        stageAttempt: 1,
        blockedReason: null,
      ...
      finalizeRunArtifact(projectName, run.runId, {
        ...
        acceptanceChecks:
          stage === "architecture" ? ["Workspace files materialized"] : [],
        modelInfo: {
          provider: this.state.state.llmProvider || process.env.LLM_PROVIDER,
          model,
        },
      ...
      log(
        stageInfo.agentKey,
        `✓ "${projectName}" → moved to "${stageInfo.next}"`,
      );
