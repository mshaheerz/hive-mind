  ...
  return (
    <div className="flex flex-col overflow-hidden rounded-lg h-[400px] bg-[rgba(20,45,69,0.8)] border border-(--border-dim)">
      ...
      <div className="flex-1 overflow-y-auto p-3">
        {sorted.length === 0 && (
          <div className="p-4 text-[14px] text-center text-(--text-dim)">
  ...
  return (
    <div className="p-2 mb-1.5 bg-black/25 border rounded animate-[fadeSlideUp_0.3s_ease]" style={{ borderColor: `${cfg.color}35`, borderLeft: `3px solid ${cfg.color}` }}>
      <div className="flex items-start justify-between gap-2 mb-1">
        <div className="font-display font-bold text-[14px] max-w-[75%] leading-normal" style={{ color: 'var(--text-primary)' }}>
          {cfg.icon} {proposal.title}
        </div>
        <span className="text-[14px] text-(--text-dim) shrink-0">{timeAgo(proposal.proposedAt)}</span>
      </div>

      <div className="flex items-center justify-between">
        <span className="text-[14px] px-1.5 rounded font-semibold border" style={{ background: `${cfg.color}15`, borderColor: `${cfg.color}50`, color: cfg.color }}>
          {cfg.label}
        </span>
        <div className="flex items-center gap-1.5">
          {proposal.complexity && (
            <span className="text-[14px] text-(--text-dim)">{proposal.complexity}</span>
          )}
          {proposal.decision?.overall && (
            <span className="text-[14px] font-semibold" style={{ color: '#ffd93d' }}>{proposal.decision.overall}/10</span>
          )}
        </div>
      </div>

      {proposal.revisionFeedback && (
        <div className="mt-0.75 text-[14px] italic leading-[1.4]" style={{ color: '#f97316' }}>
