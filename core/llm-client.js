    ...
    for (const model of modelChain) {
      try {
        ...
        if (model !== primaryModel) {
          console.log(`  [${agentName.toUpperCase()}] Used ${this.provider} fallback model: ${model}`);
        }
        return result;
      } catch (err) {
        lastError = err;
        ...
        console.log(`  [${agentName.toUpperCase()}] ${model} unavailable (${firstLine}). Trying next...`);
      }
    }

    throw new Error(`All ${this.provider} models failed for ${agentName}: ${lastError?.message}`);
  }
  ...
  async _callWithRetry(model, messages, opts = {}, maxRetries = 3) {
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (this._isRateLimitError(err) && attempt < maxRetries) {
