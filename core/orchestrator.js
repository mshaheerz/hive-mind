...
const { ScoutAgent, ForgeAgent, LensAgent, PulseAgent, EchoAgent, AtlasAgent, SageAgent, NovaAgent } = require('../agents/agents');
...
class HiveOrchestrator {
  ...
  async runTask(taskDescription) {
    ...
    const architecture = await this.agents.atlas.design(taskDescription, research);
    console.log(architecture);

    // 4. FORGE implements
    console.log('\n⚒️ FORGE is implementing...\n');
    const code = await this.agents.forge.implement(taskDescription, architecture, research);
    console.log(code);
    ...
    // 8. Save project
    this._saveProjectOutput(taskDescription, { research, architecture, code, review, tests, docs });
  ...
  async runProject(projectName) {
    ...
    if (status.stage === 'new') {
      ...
      this._updateProjectStatus(projectName, { 
        stage: decision.decision === 'APPROVED' ? 'approved' : decision.decision.toLowerCase(),
  ...
  async _continueProjectPipeline(projectName, readme, status) {
    ...
    const stages = ['research', 'architecture', 'implementation', 'review', 'tests', 'docs', 'launch'];
    const currentIdx = stages.indexOf(status.stage === 'approved' ? 'research' : status.stage);
    ...
    for (let i = Math.max(0, currentIdx); i < stages.length; i++) {
      ...
      switch (stage) {
        ...
        case 'launch':
          output = await this.agents.echo.createLaunchContent({ name: projectName, description: readme });
          fs.writeFileSync(path.join(outputDir, 'launch.md'), output);
          break;
      }

      this._updateProjectStatus(projectName, { stage, completedAt: new Date().toISOString() });
  ...
  async runApexReview() {
    ...
    this._saveQueue(this._loadQueue().map(p => {
  ...
  async _createProjectFolder(proposal) {
    const slug = proposal.title.toLowerCase().replace(/[^a-z0-9]+/g, '-').slice(0, 50);
...
`;
  ...
  async runInteractive() {
    ...
    switch (choice.trim()) {
      ...
      case '3':
        ...
        await this.submitProposal({ title, description: desc, proposedBy: 'human' });
        break;
      case '4':
  ...
  async chatWithAgent(agentName) {
    ...
    if (!agent) {
      console.error(`Unknown agent: ${agentName}. Valid: ${Object.keys(this.agents).join(', ')}`);
      return;
    }
  ...
  _updateProjectStatus(projectName, updates) {
    ...
    if (fs.existsSync(statusFile)) {
      try { status = JSON.parse(fs.readFileSync(statusFile)); } catch {}
    }
